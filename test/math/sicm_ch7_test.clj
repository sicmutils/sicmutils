...
(deftest section-1
  (let [h (compose cube sin)
    ...
    (is (= '(expt (sin a) 3) (pe (h 'a))))
    (is (= 0 (pe ((- (+ (square sin) (square cos)) 1) 'a))))
    (is (= '(f x) (pe ((literal-function 'f) 'x))))
    (is (= '(f (g x))
           (pe ((compose (literal-function 'f) (literal-function 'g)) 'x))))
...
(deftest section-3
  (let [derivative-of-sine (D sin)]
    (is (= '(cos x) (pe (derivative-of-sine 'x)))))
  (is (= '(+ (((expt D 2) f) x) (* -1 (f x)))
         (pe (((* (- D 1) (+ D 1)) (literal-function 'f)) 'x)))))
