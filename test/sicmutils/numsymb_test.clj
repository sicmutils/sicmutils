...
(deftest numsymb-test
  ...
  (testing "trig shortcuts - sin"
    (is (= 0 (g/sin 0)))
    (is (= 0 (simplify-and-freeze (g/sin 'pi))))
    (is (= 0 (simplify-and-freeze (g/sin 'two-pi))))
    (is (= 0 (simplify-and-freeze (g/sin '-pi))))
    (is (near 0.0 (g/sin Math/PI)))
    (is (near 0.0 (g/sin (* 2 Math/PI))))
    (is (near 0.0 (g/sin (- Math/PI))))
    (is (= 1 (simplify-and-freeze (g/sin 'pi-over-2))))
    (is (= 1.0 (g/sin (/ Math/PI 2)))))
  (testing "trig shortcuts - cos"
    (is (= 1 (g/cos 0)))
    (is (= -1 (simplify-and-freeze (g/cos 'pi))))
    (is (near -1.0 (g/cos Math/PI)))
    (is (= 1 (simplify-and-freeze (g/cos 'two-pi))))
    (is (near 1.0 (g/cos (* 2 Math/PI))))
    (is (= -1 (simplify-and-freeze (g/cos '-pi))))
    (is (= 0 (simplify-and-freeze (g/cos 'pi-over-2)))))
  (testing "trig shortcuts - tan"
    (is (= 0 (g/tan 0)))
    (is (= 1 (simplify-and-freeze (g/tan 'pi-over-4))))
    (is (= -1 (simplify-and-freeze (g/tan '-pi-over-4))))
...
(deftest arithmetic
  ...
  (testing "zero/one elimination"
    ...
    (is (= 'x (g/divide 'x 1.0)))
    (is (= 'x (g/divide 'x 1)))
    (is (= 0 (simplify-and-freeze (g/divide 0 'x))))
    (is (= 0 (simplify-and-freeze (g/* 0 'x))))
    (is (= 0 (simplify-and-freeze (g/* 'x 0))))
  ...
  (testing "expt"
    ...
    (is (= 1 (simplify-and-freeze (g/expt 1 'x))))
    (is (= 1 (simplify-and-freeze (g/expt 'x 0))))
    (is (= 'x (simplify-and-freeze (g/expt 'x 1))))
    (is (= 'x (simplify-and-freeze (g/expt (g/sqrt 'x) 2))))
