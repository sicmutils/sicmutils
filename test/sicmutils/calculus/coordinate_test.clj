...
(deftest various-manifold-operations
  ...
  (let-coordinates [[x y] R2-rect
    ...
    (let [mr ((point R2-rect) (up 'x0 'y0))
      ...
      (is (= 1 (simplify-and-freeze ((circular theta) mr))))
      (is (= 0 (simplify-and-freeze ((dr circular) mr))))
      (is (= 1 (((d r) d:dr) mr)))
      ...
      (is (= 0 (simplify-and-freeze ((residual vr vr) mr))))
      (is (= 0 (simplify-and-freeze ((residual vp vp) mr))))
      (is (= 0 (simplify-and-freeze ((residual vp vp) mp))))
      (is (= 0 (simplify-and-freeze ((residual vr vr) mp)))))))
