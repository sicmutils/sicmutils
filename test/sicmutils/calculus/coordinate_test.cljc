...
(deftest smoke
  (let-coordinates [[x y] R2-rect
                    [r theta] R2-polar]
    (let [p ((m/point m/R2-rect) (up 1 2))]
...
(deftest coordinates
  (testing "using coordinates"
    (using-coordinates
     ...
     (using-coordinates
      (up r theta) R2-polar
      (let [R2-polar-chi-inverse (m/point m/R2-polar)
  ...
  (testing "with-coordinate-prototype"
    (let [A m/R2-rect
          B (m/with-coordinate-prototype m/R2-rect '[X Y])]
...
(deftest various-manifold-operations
  ...
  (let-coordinates [[x y] R2-rect
                    [r theta] R2-polar]
    (let [mr ((m/point m/R2-rect) (up 'x0 'y0))
          mp ((m/point m/R2-polar) (up 'r0 'theta0))
          ...
          vp (vf/literal-vector-field 'v m/R2-polar)
          vr (vf/literal-vector-field 'v m/R2-rect)]
