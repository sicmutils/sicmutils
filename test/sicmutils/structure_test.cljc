...
(ns sicmutils.structure-test
  "TODO: Figure out `ref`, and what we are actually trying to test."
...
(deftest structures
  (testing "type"
    ...
    (is (= ::s/down (v/kind (s/down (s/up 1 2) (s/up 2 3))))))

  (testing "s+t"
    (is (= (+ (s/up 1 2) (s/up 2 3)) (s/up 3 5)))
    (is (= (+ (s/down 3 4) (s/down 1 2)) (s/down 4 6)))
    (is (= (s/down (+ 'u 4) (+ 2 'v)) (+ (s/down 'u 2) (s/down 4 'v)))))

  (testing "s-t"
    (is (= (- (s/up 1 2) (s/up 2 3)) (s/up -1 -1)))
    (is (= (- (s/down 8 5) (s/down 4 -1)) (s/down 4 6)))
    (is (= (- (s/down 8 5)) (s/down -8 -5)))
    (is (= (- (s/up 10 10) (s/up 2 3) (s/up 3 4)) (s/up 5 3))))
  ...
  (comment
    (testing "function - rotate about x axis"
      (is (= (s/up 0 0 1) ((Rx 'pi-over-2) (s/up 0 1 0))))
      (is (= (s/up 'x (sicmutils.generic/- 'z) 'y) ((Rx 'pi-over-2) (s/up 'x 'y 'z))))))
...
(deftest structural-operations
  ...
  (testing "mapr - two arg fn"
    (let [S (s/down 'a 'b (s/up 'c 'd) (s/down 'e (s/down 'f 'g)) 'h)]
      (is (= (s/down '(a 0) '(b 1) (s/up '(c 2 0) '(d 2 1)) (s/down '(e 3 0) (s/down '(f 3 1 0) '(g 3 1 1))) '(h 4))
  ...
  (testing "assoc-in"
    (is (= (s/up 4 55 6) (s/structure-assoc-in (s/up 4 5 6) [1] 55)))
    (is (= (s/down (s/up 1 22) (s/up 3 4)) (s/structure-assoc-in (s/down (s/up 1 2) (s/up 3 4)) [0 1] 22))))

  (testing "access-chains"
    (is (= (s/up [0] [1] [2]) (s/structure->access-chains (s/up 1 2 3))))
    (is (= (s/up [0] (s/up [1 0] [1 1]) (s/down [2 0] [2 1]))
           (s/structure->access-chains (s/up 't (s/up 'x 'y) (s/down 'p_x 'p_y)))))
    (is (= (s/up (s/down (s/up [0 0 0] [0 0 1]) (s/up [0 1 0] [0 1 1]))
                 (s/down (s/up [1 0 0] [1 0 1]) (s/up [1 1 0] [1 1 1])))
           (s/structure->access-chains (s/up (s/down (s/up 1 2) (s/up 2 3)) (s/down (s/up 3 4) (s/up 4 5)))))))
      ...
      (let [vs (velocity-tuple
            ...
            L1 (fn [[v1 v2]]
                 (+ (* 1/2 'm1 (square v1))
                    (* 1/2 'm2 (square v2))))]
        (is (= '(s/down
                 (s/down (s/down (s/down m1 0) (s/down 0 0)) (s/down (s/down 0 m1) (s/down 0 0)))
                 (s/down (s/down (s/down 0 0) (s/down m2 0)) (s/down (s/down 0 0) (s/down 0 m2))))
