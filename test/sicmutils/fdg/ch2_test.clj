...
(deftest section-2-2
  (let [R2->R '(-> (UP Real Real) Real)
        f-rect (literal-function 'f-rect R2->R)
        f (compose f-rect R2-rect-chi)
    ...
    (is (= '(f-rect (up x0 y0)) (simplify-and-freeze (f-rect (up 'x0 'y0)))))
    (is (= '(g-rect (up x0 y0)) (simplify-and-freeze (g R2-rect-point))))
    (is (= '(g-rect (up x0 y0)) (simplify-and-freeze (g corresponding-polar-point))))
    (let-coordinates [[x y] R2-rect
                      [r theta] R2-polar]
      (is (= 'x0 (x (R2-rect-chi-inverse (up 'x0 'y0)))))
      (is (= (* 'r0 (cos 'theta0)) (simplify (x (R2-polar-chi-inverse (up 'r0 'theta0))))))
      (is (= 'r0 (simplify-and-freeze (r (R2-polar-chi-inverse (up 'r0 'theta0))))))
