...
(deftest series-test
  (testing "basics"
    (let [Q (series/starting-with 4)
      ...
      (is (= '(ε (* 2 ε) (* 3 ε) (* 4 ε)) (g/simplify (series/take 4 (* nats 'ε)))))
      (is (= '(ε (* 2 ε) (* 3 ε) (* 4 ε)) (g/simplify (series/take 4 (* 'ε nats)))))
      (is (= '(0 -2 -6 -12)
  ...
  (let [S (series/starting-with (literal-function 'f)
    ...
    (testing "with functions"
      (is (= '[(* (f x) (sin x)) (* (sin x) (g x)) 0 0]
             (g/simplify (series/take 4 ((* S sin) 'x)))))
      (is (= '[(* (f x) (sin x)) (* (sin x) (g x)) 0 0]
             (g/simplify (series/take 4 ((* sin S) 'x))))))
...
(deftest taylor
  (let [taylor-series-expander (fn [f x h]
    ...
    (is (= '(+ (* 1/24 (expt dx 4) (sin x))
               ...
               (* dx (cos x))
               (sin x))
           (g/simplify (reduce + (series/take 5 (taylor-series-expander sin 'x 'dx))))))
    (is (= '(1
             (* 1/2 dx)
             (* -1/8 (expt dx 2))
             (* 1/16 (expt dx 3))
             (* -5/128 (expt dx 4))
             (* 7/256 (expt dx 5)))
           (g/simplify (series/take 6 (taylor-series-expander #(sqrt (+ 1 %)) 0 'dx)))))))
