...
(ns net.littleredcomputer.sicmutils.function-test
...
(deftest function-basic
  (let [f (literal-function 'F)]
    ...
    (testing "kind"
      (is (= :net.littleredcomputer.sicmutils.function/function (v/kind f))))
    (testing "arity > 1"
...
(deftest function-algebra
  (let [add2 (fn [x] (g/+ x 2))
    ...
    (testing "binary"
      ...
      (is (= :net.littleredcomputer.sicmutils.value/function (v/kind (g/expt add2 2)))))
    (testing "arity 2"
      (let [f (fn [x y] (+ x y))
            g (fn [x y] (* x y))
            h (g/+ f g)
