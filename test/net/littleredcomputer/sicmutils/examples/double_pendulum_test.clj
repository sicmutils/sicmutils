...
(ns net.littleredcomputer.sicmutils.examples.double-pendulum-test
  (:refer-clojure :exclude [+ - * / zero? partial])
  (:require [net.littleredcomputer.sicmutils.env :refer :all]
            [net.littleredcomputer.sicmutils.mechanics.lagrange :refer :all]
            [net.littleredcomputer.sicmutils.examples.double-pendulum :as double]
...
(deftest equations
  (let [state (up 't (up 'θ 'φ) (up 'θdot 'φdot))
    ...
    (is (= '(+ (* -1 (cos θ) g l1 m1)
               ...
               (* -1 (cos φ) g l2 m2))
           (simplify (V state))))
    (is (= '(/ (+ (* 2 (cos (+ θ (* -1 φ))) l1 l2 m2 θdot φdot)
                  (* (expt l1 2) m1 (expt θdot 2))
                  (* (expt l1 2) m2 (expt θdot 2))
                  (* (expt l2 2) m2 (expt φdot 2)))
               2N)
           (simplify (T state))))
    (is (= '(/
             (+
              (* 2 (cos (+ θ (* -1 φ))) l1 l2 m2 θdot φdot)
              (* (expt l1 2) m1 (expt θdot 2))
              (* (expt l1 2) m2 (expt θdot 2))
              (* (expt l2 2) m2 (expt φdot 2))
              (* 2 (cos θ) g l1 m1)
