...
(ns net.littleredcomputer.sicmutils.expression
  (:require [net.littleredcomputer.sicmutils.value :as v]))
...
(defn walk-expression
  ...
  (fn walk [x]
    (cond (variable? x) (if-let [binding (environment x)]
                        (if-not (= :net.littleredcomputer.sicmutils.value/function (v/kind binding)) binding x)
                        (throw (IllegalArgumentException.
                                (str "no binding for " x " found."))))
          (number? x) x
          (instance? Expression x) (walk (expression-of x))
          (sequential? x) (let [f (environment (first x))]
                            (when-not (= :net.littleredcomputer.sicmutils.value/function (v/kind f))
                              (throw (IllegalArgumentException.
