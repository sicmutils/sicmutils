      ...
      [Object
       ...
       (equals [_ b]
               (and (instance? RationalFunction b)
                    (and (= arity (.-arity ^RationalFunction b))
                         (= u (.-u ^RationalFunction b))
                         (= v (.-v ^RationalFunction b)))))]
...
(defn make
  ...
  {:pre [(p/polynomial? u)
         (p/polynomial? v)
         (= (.-arity ^Polynomial u) (.-arity ^Polynomial v))]}
  (when (v/zero? v)
...
(defn add
  ...
  (let [a (.-arity r)
        ...
        d1 (poly/gcd u' v')]
    (if (v/one? d1)
      (p :rf/add-first (make-reduced a (p/add (p/mul u v') (p/mul u' v)) (p/mul u' v')))
      (p :rf/add-second
...
(def ^:private operator-table
  {'+ (fn ([] 0)
        ([x] x)
        ([x y] (p :rf/add (g/add x y)))
        ([x y & more]
         (p :rf/add* (reduce g/add (p :rf/add-init (g/add x y)) more))))
   '- (fn [arg & args]
        (p :rf/sub (if (some? args) (g/sub arg (reduce g/add args)) (g/negate arg))))
   '* (fn
        ([] 1)
        ([x] x)
        ([x y] (p :rf/mul2 (g/mul x y)))
        ([x y & more]
         (p :rf/mul (reduce g/mul (g/mul x y) more))))
   '/ (fn [arg & args]
        (p :rf/div (if (some? args)
                     (g/div arg (reduce g/mul args))
                     (g/invert arg))))
   'negate negate
   'invert invert
   'expt g/expt
...
(deftype RationalFunctionAnalyzer [polynomial-analyzer]
  ...
  (expression-> [this expr cont v-compare]
    ...
    (p :rf/expression->
       (let [expression-vars (p :rf/vars (sort v-compare (set/difference (x/variables-in expr) operators-known)))
             arity    (count expression-vars)
             sym->var (zipmap expression-vars (a/new-variables this arity))
             _ (def cake sym->var)
             _ (def cakef expr)
             expr' (p :x/eval (x/evaluate expr sym->var operator-table))
             expr' (p :x/eval2 (x/evaluate expr sym->var operator-table))]
...
(do (defmethod g/add [::rational-function ::rational-function] [a b] (add a b))
