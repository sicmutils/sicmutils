...
(defn- sum:l
  "Returns the Kahan summation sum of all real values generated by mapping the
  real-valued function `f` across the same-indexed entries of all supplied
  structures, one level deep."
  [f [s :as structs]]
  (ua/sum (fn [i]
...
(defn- sum:r:l
  "Accepts some real-valued function `f` and a sequence of isomorphic
  `structures`; returns the sum of the values returned from applying `f` to each
  associated set of entries in each input structure."
  [f structures]
  (sum:l (fn [& elements]
...
(defn sumr
  "Given some real-valued function `f` and any number of isomorphic `structures`,
  returns the real sum of the results of applying `f` to each associated set of
